...
| Review my cleanup/topic trackers | Summarize cleanup reports, cleanup tasks, and topic task notes in one place, sort them by pending/done/snapshot status, open a small priority dashboard, save that dashboard as a reusable tracking note, then archive or batch-sync those tracking notes later | `/notes/tracking-overview`, `/notes/tracking-status`, `/notes/tracking-dashboard`, `/notes/tracking-note`, `/notes/tracking-note-refresh`, `/notes/tracking-note-archive`, `/notes/tracking-note-sync`, `/notes/get` |
...
| Organize folders or tags | Inspect folder tags, hierarchy, move one note, generate a cleanup plan, turn it into action suggestions, preview or apply the next cleanup step, batch-apply the first few write-capable cleanup actions, save a cleanup runbook, save a cleanup execution report, refresh an existing cleanup report note, create a checklist-style cleanup task note, refresh that task note, or review all tracking notes in one overview, status board, priority dashboard, saved tracking note, archived note, or batch-sync flow | `/folders/list`, `/folders/stats`, `/folders/relations`, `/folders/move-note`, `/notes/cleanup-plan`, `/notes/cleanup-actions`, `/notes/cleanup-preview`, `/notes/cleanup-runbook`, `/notes/cleanup-note`, `/notes/cleanup-apply`, `/notes/cleanup-batch-apply`, `/notes/cleanup-report`, `/notes/cleanup-report-refresh`, `/notes/cleanup-task`, `/notes/cleanup-task-refresh`, `/notes/tracking-overview`, `/notes/tracking-status`, `/notes/tracking-dashboard`, `/notes/tracking-note`, `/notes/tracking-note-refresh`, `/notes/tracking-note-archive`, `/notes/tracking-note-sync`, `/notes/batch-update` |
