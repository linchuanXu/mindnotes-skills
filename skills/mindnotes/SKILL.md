...
| Organize folders or tags | Inspect folder tags, hierarchy, move one note, generate a cleanup plan, turn it into action suggestions, preview or apply the next cleanup step, batch-apply the first few write-capable cleanup actions, save a cleanup runbook, save a cleanup execution report, refresh an existing cleanup report note, or create a checklist-style cleanup task note | `/folders/list`, `/folders/stats`, `/folders/relations`, `/folders/move-note`, `/notes/cleanup-plan`, `/notes/cleanup-actions`, `/notes/cleanup-preview`, `/notes/cleanup-runbook`, `/notes/cleanup-note`, `/notes/cleanup-apply`, `/notes/cleanup-batch-apply`, `/notes/cleanup-report`, `/notes/cleanup-report-refresh`, `/notes/cleanup-task`, `/notes/batch-update` |
